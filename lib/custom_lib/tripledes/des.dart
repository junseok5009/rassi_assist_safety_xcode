import 'package:rassi_assist/custom_lib/tripledes/constants.dart';
...
class DESEngine extends BaseEngine {
  ...
  void init(bool forEncryption, List<int> key) {
    ...
    var keyBits = List<int>.filled(56, 0);
    for (var i = 0; i < 56; i++) {
    ...
    for (var nSubKey = 0; nSubKey < 16; nSubKey++) {
      // Create subkey
      var subKey = subKeys[nSubKey] = List.generate(24, (_) => 0);
  ...
  void exchangeLR(offset, mask) {
    var t =
    (((rightShift32(_lBlock, offset)).toSigned(32) ^ _rBlock) &
    mask)
        .toSigned(32);
  ...
  void exchangeRL(offset, mask) {
    var t =
    (((rightShift32(_rBlock, offset)).toSigned(32) ^ _lBlock) & mask).toSigned(32);
