...
/// 2023.07
/// Android 푸시 MethodChannel 을 이용한 연동
class PushAosService {
  static const String TAG = "[PushAosService] ";
  static late PushAosService _instance;
  factory PushAosService() => _instance ?? PushAosService._internal();

  static const channel = MethodChannel(Const.METHOD_CHANNEL_PUSH);

  //명명된 생성자 (private 으로 명명된 생성자)
  PushAosService._internal() {
    DLog.d(TAG, "PushAosService._internal()");
    _instance = this;
    _loadPrefData();
  }
  ...
  _loadPrefData() async {
    DLog.d(PushAosService.TAG, '_loadPrefData() id : $_userId');
  ...
  void _initConnection() async {
    if(Platform.isAndroid) {
      DLog.d(PushAosService.TAG, 'InApp Init Connection');
  ...
  void _setChannelListener() async {
    channel.setMethodCallHandler((call) async {
      if (call.method == 'billing_ok') {
        ...
        String data = call.arguments;
        // 전달받은 데이터 처리
        DLog.d(PushAosService.TAG, '# 상품정보 가격정보 => $data');
        return "데이터 전달 완료"; // 데이터 처리 결과를 Android로 전달
      }
      else if (call.method == 'getFormatedPrice') {
        String data = call.arguments;
  ...
  void _handleBillingResponse(String data) {
    DLog.d(PushAosService.TAG, '# 빌링_handleBillingResponse => $data');
  ...
  void _handleStoreCode(String data) {
    DLog.d(PushAosService.TAG, '# 빌링_handleStoreCode => $data');
    ...
    if(data != null) {
      ...
      if (strCode == 'USER_CANCELED') {
        DLog.d(PushAosService.TAG, '# 빌링_사용자_취소');
        //사용자 취소일 경우 아무것도 안함
...
/*  void _handlePurchaseError(PurchaseResult purchaseError) {
  ...
  void requestPurchaseAsync() async {
    if(Platform.isAndroid) {
      DLog.d(PushAosService.TAG, '# requestPurchaseAsync 요청 -> channel');
  ...
  void _fetchPosts(String trStr, String json) async {
    DLog.d(PushAosService.TAG, '$trStr $json');
  ...
  void _parseTrData(String trStr, final http.Response response) {
    DLog.d(PushAosService.TAG, response.body);
    _callProStatusChangedListeners('md_exit');
