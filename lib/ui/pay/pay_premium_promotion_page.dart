...
class PayPremiumPromotionPage extends StatelessWidget {
  ...
  const PayPremiumPromotionPage({super.key});
  // 계정결제	LPH1
  // 계정결제_처음50할인	LPH7
  // 계정결제_처음40할인	LPH8
  // 계정결제_처음30할인	LPH9
  ...
  Widget build(BuildContext context) {
    ...
    return MediaQuery(
      ...
      child: Scaffold(
        ...
        body: PayPremiumPromotionWidget(args.data),
      ),
    );
  }
}

class PayPremiumPromotionWidget extends StatefulWidget {

  final String _intentData;
  PayPremiumPromotionWidget(this._intentData);

  @override
  State<StatefulWidget> createState() => PayPremiumPromotionState(_intentData);
}

class PayPremiumPromotionState extends State<PayPremiumPromotionWidget> {

  final String _intentData;
  PayPremiumPromotionState(this._intentData);
  ...
  void initState() {
    super.initState();
    DLog.d(PayPremiumPromotionPage.TAG, 'initState()');
    CustomFirebaseClass.logEvtScreenView(PayPremiumPromotionPage.TAG_NAME);
  ...
  _loadPrefData() async {
    ...
    setState(() {
      ...
      DLog.d(PayPremiumPromotionPage.TAG, "_curProd : $_curProd");
      if (_userId == '') {
        Navigator.pop(context);
      } else {
        _fetchPosts(
      ...
      case 'ad3':
        {
          ...
          _buyInfo = '★ 첫달 50% 할인혜택($_priceOnce)과 둘째달 부터는 '
              '23% 할인혜택($_priceOriginal)을 모두 드립니다.\n★ 정기결제는 언제든 구독을 취소하실 수 있어요.';
          break;
        }
      case 'ad4':
        {
      ...
      case 'at1':
        {
          ...
          _buyInfo = '★ 결제 부담없이 무료체험으로 먼저 만나보세요.\n★ 무료체험 기간 중 '
              '언제든 가입을 해지하실 수 있으며, 결제 전 계정을 해지하시면 요금이 부과되지 않습니다';
          break;
        }
      case 'at2':
        {
    ...
    return Column(
      ...
      children: const [
        ...
        Padding(
          ...
          child: Text(
            "혼자하는 투자가 어려우세요?\n대한민국 대표 AI의 전문적인 종목분석과 관리를 받무보세요.\n라씨 매매비서는 투자를 쉽게 만들어 드립니다.",
  ...
  Future _getProduct() async {
    ...
    DLog.d(PayPremiumPromotionPage.TAG, '_getProduct() _intentData : $_intentData');
    ...
    if (trStr == TR.USER04) {
      ...
      if (resData.retCode == RT.SUCCESS) {
        ...
        _fetchPosts(
            ...
            jsonEncode(<String, String>{
              'userId': _userId,
              'viewPage': PayPremiumPromotionPage.VIEW_PAGE_CODE,
              'promoDiv': '',
            }));
      }
    }
