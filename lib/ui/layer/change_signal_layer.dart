...
class ChangeSignalLayer extends StatelessWidget {
  //const ChangeSignalLayer({super.key});
  final StockPktSignal? stockPktSignal;

  const ChangeSignalLayer({Key? key, this.stockPktSignal}) : super(key: key);
...
class ChangeSignalLayerView extends StatefulWidget {
  final StockPktSignal? stockPktSignal;

  const ChangeSignalLayerView({Key? key, this.stockPktSignal}) : super(key: key);

  @override
  State<ChangeSignalLayerView> createState() => _ChangeSignalLayerViewState();
}

class _ChangeSignalLayerViewState extends State<ChangeSignalLayerView> {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) {
      _stockInfoProvider.postRequest(widget.stockPktSignal!.stockCode);
    });
  }
            ...
            if (stockInfoProvider.getIsLoading) {
              ...
              return Consumer<SignalLayerSliderProvider>(
                builder: (_, provider, __) {
                  SchedulerBinding.instance.addPostFrameCallback((_) {
                    _selectPriceViewHeight = CommonFunctionClass.instance.getSize(_containerKey).height;
                  });
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
  ...
  Widget get _setMakeSignalBtn => InkWell(
        onTap: () async {
          if (_isSelectPrice && _isSelectPriceTouchOnce) {
            ...
            if (widget.stockPktSignal!.resultDiv == 'S') {
              result = await Provider.of<SignalProvider>(context, listen: false).changeSignalS(
                widget.stockPktSignal!.pocketSn,
                widget.stockPktSignal!.stockCode,
                Provider.of<SignalLayerSliderProvider>(context, listen: false).getCurrentPrice.toStringAsFixed(0),
              );
            } else if (widget.stockPktSignal!.resultDiv == 'P') {
              result = await Provider.of<SignalProvider>(context, listen: false).changeSignalP(
                widget.stockPktSignal!.pocketSn,
                widget.stockPktSignal!.stockCode,
                Provider.of<SignalLayerSliderProvider>(context, listen: false).getCurrentPrice.toStringAsFixed(0),
              );
