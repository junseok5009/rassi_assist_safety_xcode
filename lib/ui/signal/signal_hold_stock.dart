...
class SignalHoldPage extends StatefulWidget {
  ...
  static const String TAG_NAME = '매매신호_보유중_전체';

  @override
  State<StatefulWidget> createState() => SignalHoldPageState();
}

class SignalHoldPageState extends State<SignalHoldPage> {
  ...
  void initState() {
    super.initState();
    CustomFirebaseClass.logEvtScreenView(SignalHoldPage.TAG_NAME);
    _scrollController = ScrollController();
    _scrollController.addListener(_scrollListener);

    _loadPrefData();
    Future.delayed(const Duration(milliseconds: 300), () {
      DLog.d(SignalHoldPage.TAG, "delayed user id : $_userId");
      if (_userId != '') {
        _requestData();
      }
    });
  ...
  _requestData() {
    DLog.d(SignalHoldPage.TAG, '### requestData');
    _fetchPosts(
        TR.SIGNAL06,
        jsonEncode(<String, String>{
          'userId': _userId,
          'tradeFlag': 'H', //H:보유, W:관망
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                ...
                                Text(TStyle.getDateTdFormat(
                                    subList[index].tradeDttm)),
                              ],
                            )
                          ],
                        ),
  ...
  void _fetchPosts(String trStr, String json) async {
    DLog.d(SignalHoldPage.TAG, '$trStr $json');
  ...
  void _parseTrData(String trStr, final http.Response response) {
    DLog.d(SignalHoldPage.TAG, response.body);
