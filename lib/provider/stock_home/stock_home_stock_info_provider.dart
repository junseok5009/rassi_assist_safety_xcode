...
class StockInfoProvider with ChangeNotifier {
  ...
  void postRequestDiv(String vStockCode, String selectDiv) {
    if (!_isLoading) {
      _isLoading = true;
      notifyListeners();
    }
    _fetchPosts(vStockCode, selectDiv);
  }

  void _fetchPosts(String vStockCode, String selectDiv) async {
    ...
    try {
      final http.Response response = await http.post(
        ...
        body: jsonEncode(<String, String>{
          ...
          if (selectDiv.isNotEmpty) 'selectDiv': selectDiv,
        }),
      ...
      DLog.e('response.body : ${response.body}');
      if (resData.retCode == RT.SUCCESS) {
        _search01 = resData.retData!;
        if (_search01.fluctuationRate.contains('-')) {
          _curSubInfo =
              '▼${TStyle.getMoneyPoint(_search01.fluctuationAmt.replaceAll('-', ''))}  ${_search01.fluctuationRate}%';
          _color = RColor.sigSell;
        } else if (_search01.fluctuationRate == '0.00') {
          _curSubInfo =
              '${TStyle.getMoneyPoint(_search01.fluctuationAmt)}  ${_search01.fluctuationRate}%';
        ...
        _timeTxt =
            '${TStyle.getDateDivFormat(_search01.tradeDate)} ${TStyle.getTimeFormat(_search01.tradeTime)} ${_search01.timeDivTxt}';
