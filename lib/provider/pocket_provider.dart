...
class PocketProvider with ChangeNotifier {
  ...
  void loggingPocketInfo() {
    _pktList.asMap().forEach((key1, value1) {
      ...
      DLog.d('ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ', 'ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ');
    });
  }
  ...
  Future<bool> setList() async {
    List<Pocket> getPcoketList = await PocketProviderNetwork.instance.getList();
    _pktList.clear();
  ...
  Future<bool> addPocket(String newPocketName) async {
    Pocket newPocket = (await PocketProviderNetwork.instance.addPocket(newPocketName)) as Pocket;
  ...
  Future<bool> changeNamePocket(Pocket pocket) async {
    bool result = await PocketProviderNetwork.instance.changeNamePocket(pocket);
    if (result) {
      int findIndex = _pktList.indexWhere((findPocket) => findPocket.pktSn == pocket.pktSn);
      if (findIndex > -1) {
  ...
  Future<bool> changeOrderPocket(List<Pocket> listPocket) async {
    bool result = await PocketProviderNetwork.instance.changeOrderPocket(listPocket);
    if (result) {
      _pktList.clear();
  ...
  Future<bool> deleteListPocket(List<Pocket> listPocket) async {
    ...
    bool result = await PocketProviderNetwork.instance.deleteListPocket(listPocket);
    if (result) {
      _pktList.removeWhere((element) {
        return listPocket.any((deletePocket) => deletePocket.pktSn == element.pktSn);
      });
      notifyListeners();
      return true;
    } else {
  ...
  // 24.07.16 포켓 안에 종목 추가 / return [refresh / fail / retMsg] [return type 신규]
  Future<PocketApiResult> addStock(Stock newStock, String pocketSn) async {
    String result = await PocketProviderNetwork.instance.addStock(newStock, pocketSn);
  ...
  Future<String> deleteStock(Stock delStock, String pocketSn) async {
    ...
    if (result == CustomNvRouteResult.refresh) {
      ...
      if (pocketListIndex == -1) {
        ...
        int stockListIndex = _getStockListIndexByStockCode(pocketListIndex, delStock.stockCode);
        if (stockListIndex == -1) {
          await setList();
          return CustomNvRouteResult.fail;
        } else {
  ...
  Future<bool> changeOrderStock(String pocketSn, List<Stock> listStock) async {
    bool result = await PocketProviderNetwork.instance.changeOrderStock(pocketSn, listStock);
    if (result) {
      try {
        Pocket pocket = _pktList.singleWhere((element) => element.pktSn == pocketSn);
  ...
  Future<String> deleteListStock(String pocketSn, List<Stock> listStock) async {
    ...
    String result = await PocketProviderNetwork.instance.deleteListStock(pocketSn, listStock);
    if (result == CustomNvRouteResult.refresh) {
      try {
        Pocket pocket = _pktList.singleWhere((element) => element.pktSn == pocketSn);
        pocket.stkList.removeWhere((element) {
          return listStock.any((deleteStock) => deleteStock.stockCode == element.stockCode);
        });
        notifyListeners();
        return CustomNvRouteResult.refresh;
      } on Exception catch (_, __) {
  ...
  Future<String> changeAlarmListStock(List<Stock> listStock) async {
    String result = await PocketProviderNetwork.instance.changeAlarmListStock(listStock);
    notifyListeners();
    return result;
  }
  ...
  int _getStockListIndexByStockCode(int pocketListIndex, String stockCode) {
    return _pktList[pocketListIndex].stkList.indexWhere((element) => element.stockCode == stockCode);
  }
